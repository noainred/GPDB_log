...
if "dev" in mode:
    ...
    options_read_dir = '/root/stlog/'
    options_write_dir = '/root/stlog_parsing/'
else:
...
for file_name in os.listdir(options_read_dir):
    if "sys." in file_name:
        ...
        for j in range(1, nodecount + 1):
            if os.path.exists(options_write_dir + globals()[
                'sdw{}'.format(j)] + '-' + date_year + '-' + date_moth + '-' + date_day):
                os.remove(options_write_dir + globals()[
                    'sdw{}'.format(j)] + '-' + date_year + '-' + date_moth + '-' + date_day)
                print("file deleted : " + options_write_dir + globals()[
                    'sdw{}'.format(j)] + '-' + date_year + '-' + date_moth + '-' + date_day)

        for j in range(1, nodecount + 1):
            globals()['sdw{}'.format(j) + '_parsed'] = open(
                options_write_dir + globals()['sdw{}'.format(j)] + '-' + date_year + '-' + date_moth + '-' + date_day,
                'w')
