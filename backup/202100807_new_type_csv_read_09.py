...
####
def load_csv_then_df_then_import_to_gpdb(greenplumpdb, options_write_dir, run_greenplum_user_name, run_greenplum_user_pass, run_greenplum_hostname, run_greenplum_database_name, run_greenplum_tablenameNo1):
        ...
        if ".csv" in file_name:
            ...
            with open(options_write_dir + file_name) as read_csv_file:
                ...
                for i in range(0, len(data)):
                    ...
                    curs = greenplumpdb.cursor()
                    s = " "
                    s += "INSERT INTO " + run_greenplum_tablenameNo1
                    s += " ("
                    s += "date, cpu_usr, cpu_sys,  cpu_idl,  cpu_wai,  cpu_siq,  dsk_read,  dsk_writ,  io_read,  io_writ,  paging_in,  paging_out,  interrupts_42,  interrupts_45,  interrupts_46,  load_avg_1m,  load_avg_5m,  load_avg_15m,  memory_usage_used,  memory_usage_cache,  memory_usage_free,  net_recv,  net_sent,  swap_used,  swap_free "
                    s += ") VALUES ("
                    s += data[i]
                    s += ")"
...
#load_csv_then_df_then_import_to_gpdb(greenplumpdb, options_write_dir, run_greenplum_user_name, run_greenplum_user_pass, run_greenplum_hostname, run_greenplum_database_name, run_greenplum_tablenameNo1)
